...
# 2. Membuat Widget Dropdown untuk memilih Kantin
dropdown_kantin = Dropdown(options=list(database_kantin.keys()),
    value=list(database_kantin.keys())[0],
    description='Pilih Kantin:',
    disabled=False,
)

# 3. Wadah output untuk menampilkan link atau gambar
output_area = output()
...
# 5. Menghubungkan dropdown dengan fungsi aksi
dropdown_kantin.observe(ketika_kantin_dipilih, names='value')

# 6. Menampilkan antarmuka ke layar
print("--- APLIKASI PEMILIHAN MENU KANTIN ---")
display(dropdown_kantin, output_area)

# Trigger pilihan pertama saat aplikasi dibuka
ketika_kantin_dipilih({'new': dropdown_kantin.value})
